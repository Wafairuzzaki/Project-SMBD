...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($jumlah >= 20000) {
        $stmt = $conn->prepare("CALL sp_setor(?, ?)");
        $stmt->bind_param("ii", $id_akun, $jumlah);
        if ($stmt->execute()) {
            $_SESSION['user'] = getUserById($id_akun, $conn);
            $success = "Berhasil menyetor sebesar Rp" . number_format($jumlah, 0, ',', '.');
        } else {
            $error = "Gagal melakukan setoran.";
        }
    } else {
        $error = "Jumlah setoran minimal Rp20.000.";
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Setor Saldo - UniBank</title>
...
<div class="container">
    <div class="card mb-4">
        ...
        <form method="post">
            <div class="mb-3">
                <label for="jumlah" class="form-label">Jumlah Setoran (min Rp20.000):</label>
    ...
    <div class="card">
        <h4 class="mb-3">Riwayat Setoran</h4>
        <table class="table table-striped table-bordered">
            ...
            <tbody>
                <?php
                $result = $conn->query("SELECT * FROM view_riwayat_setor WHERE id_akun = $id_akun");
                ...
                <?php endwhile; else: ?>
                <tr><td colspan="2" class="text-center">Belum ada riwayat setoran</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
